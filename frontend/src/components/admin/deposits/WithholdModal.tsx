...
interface WithholdModalProps {
  onClose: () => void;
  onConfirm: (reason: string) => Promise<void>;
}

export default function WithholdModal({ onClose, onConfirm }: WithholdModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#0d2a20] border border-[#163a2c] rounded-2xl p-6 max-w-[400px] w-full">
        <h2 className="font-display text-[16px] text-white mb-1">Report damage — withhold deposit</h2>
        <p className="text-[12px] text-[#9fd8bc] mb-4">The rider will be notified via WhatsApp with this reason.</p>
        ...
        <div className="flex gap-2 mt-5">
          ...
          >
            {submitting ? 'Withholding...' : 'Withhold deposit'}
          </button>
        </div>
      </div>
    </div>
